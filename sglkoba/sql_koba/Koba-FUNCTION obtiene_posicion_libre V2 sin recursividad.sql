...
$BODY$
  LANGUAGE plpgsql VOLATILE;
--=================================================================================================================================================================================
--- sirve para arreglar datos ya creados en bd
...
BEGIN
  ...
  SELECT entbodega, entposicion, substring(entposicion from 1 for 1), right(entposicion,-1), case when substring(entposicion from '.$') ~ E'^\\d+$' then substring(entposicion from '.$')::integer  else 0 end
  INTO _bodega, _entposicion, _estante, _posicion, _nivel FROM entrada WHERE entcodsx = _entrada; 
  IF _bodega IS NOT NULL AND _posicion IS NOT NULL AND _nivel >= 0 THEN
...
BEGIN
  ...
  IF _bodega IS NOT NULL AND _entrada IS NOT NULL AND char_length(_entposicion) >= 5 THEN
    SELECT substring(_entposicion from 1 for 1), right(_entposicion,-1), case when substring(_entposicion from '.$') ~ E'^\\d+$' then substring(_entposicion from '.$')::integer  else 0 end
...
CREATE OR REPLACE FUNCTION movimiento.es_tunel(_bodega integer, _entposicion character varying ) 
  RETURNS integer AS  -- 1 ES tunel o viga, 0 LA POSICION ES VALIDA
$BODY$--
DECLARE 
  _posicion character varying;
  _estante character varying;
...
_entsaldosinnacf numeric(15,2), _entunidad character varying(3), _entlote integer, _lctrafcodsx integer)
 RETURNS int AS
...
BEGIN
	...
	INSERT INTO lctraf_entrada VALUES (DEFAULT,_lctrafcodsx,_nueva_entrada);
    ...
    UPDATE lote_contenedor_trafico set lctrafingreso = 'S' where lctrafcodsx = _lctrafcodsx;
RETURN vlr;
